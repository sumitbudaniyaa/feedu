import { Badge, Dialog, DialogContent, DialogHeader, DialogTitle, Skeleton } from '@feedo/ui';
import { formatCurrency, formatDate } from '@feedo/utils';
import { useCustomerAnalytics } from '../lib/api.js';

/** Per-diner analytics for the company portal. */
        ...
        {isLoading || !data ? (
          ...
          <div className="max-h-[65vh] space-y-5 overflow-y-auto pr-1">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              {data.firstOrderAt && <Chip>First {formatDate(data.firstOrderAt)}</Chip>}
              {data.lastOrderAt && <Chip>Last {formatDate(data.lastOrderAt)}</Chip>}
              {data.peakHour != null && <Chip>~{data.peakHour}:00</Chip>}
              <Chip>{data.rewardClaimCount} claims</Chip>
            </div>
